...
class Tester extends BaseController
{
            ...
            if ($res->getStatusCode() == '200') {
                ...
                if (!empty($dump)) {
                    if (count($dump) >= 1) {
                        ...
                        $builder->set('pnsId', $dump[0]['idPns']);
                        ...
                        $builder->set('jenisPenugasanId', $dump[0]['jenisPenugasanId']);
                        $builder->set('subJabatanId', $dump[0]['subJabatanId']);
                        $builder->set('jenisMutasiId', $dump[0]['jenisMutasiId']);
                        $builder->set('idRwyt', $dump[0]['id']);
                        $builder->set('nomorSk', $dump[0]['nomorSk']);
                        $builder->set('instansiKerjaId', $dump[0]['instansiKerjaId']);
                        $builder->set('satuanKerjaId', $dump[0]['satuanKerjaId']);
    ...
    public function unorJabatan()
    {
        ...
        $loop = $this->sipgan->table('testingekin')->where('response is NULL',NULL, TRUE)->get()->getResultArray();
        // dd($loop);
        foreach ($loop as $row) :
            ...
            $body = '{
                "eselonId":' . $eselon . ',
                "id":"' . $row["idRwyt"] . '",
                "instansiId":"' . $row["instansiKerjaId"] . '",
            ...
            $request = new Request('POST', 'https://apimws.bkn.go.id:8243/apisiasn/1.0/jabatan/save', $headers, $body);
            $res     = $client->sendAsync($request)->wait();
            $statuscode = $res->getStatusCode();
            $response   = $res->getReasonPhrase();
            //simpan hasil ke database
            $update  = "UPDATE testingekin SET response='" . $response . "', statuscode='" . $statuscode . "' WHERE nip='" . $row['NIP'] . "'";
